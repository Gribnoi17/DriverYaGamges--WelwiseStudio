...
public class ObjectMovingg : MonoBehaviour
{
        ...
        for (int i = 0; i < transform.childCount; i++)
        {
            ...
            if (child.position.z <= turnOffZCoordinate)
            {
                if (_spawnByPoints)
                    ...
                    ResetPosition(child, lastChildZ);

                lastChildZ = child.position.z;
            }
        }
    }

    private void ResetPosition(Transform pos, float lastChildZ)
    {
        pos.position = new Vector3(_startPosition.x, _startPosition.y, lastChildZ + Mathf.Abs(_moveDirection.z * pos.localScale.z));
    }
